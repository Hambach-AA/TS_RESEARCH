def CUDA_Union(name, subseqLen, fragmentLen, fragmentComp, fragmentNum, step):
    ...
    cTs1 = (ctypes.c_double * (fragmentLen + subseqLen - 1))()
    cMu1 = (ctypes.c_double * fragmentLen)()
    cSigma1 = (ctypes.c_double * fragmentLen)()
    cMp1 = (ctypes.c_double * fragmentLen)()
    cMpIndex1 = (ctypes.c_int64 * fragmentLen)()

    cTs2 = (ctypes.c_double * (fragmentLen + subseqLen - 1))()
    cMu2 = (ctypes.c_double * fragmentLen)()
    cSigma2 = (ctypes.c_double * fragmentLen)()
    cMp2 = (ctypes.c_double * fragmentLen)()
    cMpIndex2 = (ctypes.c_int64 * fragmentLen)()
    ...
    for i in range(0, fragmentComp + 1, step):
        ...
        for j in range(fragmentComp-1, fragmentNum + 1, step):
            ...
            stepTs1 = subseqLen - 1
            for io in range(0, muSigma1.shape[1], fragmentLen):

                ctypes.memmove(cTs1, ts1[0][io + stepTs1 - (subseqLen - 1):io + stepTs1 + fragmentLen].ctypes.data, ts1[0][io + stepTs1 - (subseqLen - 1):io + stepTs1 + fragmentLen].nbytes)
                ctypes.memmove(cMu1, muSigma1[0][io:io + fragmentLen].ctypes.data, muSigma1[0][io:io + fragmentLen].nbytes)
                ctypes.memmove(cSigma1, muSigma1[1][io:io + fragmentLen].ctypes.data, muSigma1[1][io:io + fragmentLen].nbytes)
                ctypes.memmove(cMp1, mp1[0][io:io + fragmentLen].ctypes.data, mp1[0][io:io + fragmentLen].nbytes)
                ctypes.memmove(cMpIndex1, mpIndex1[0][io:io + fragmentLen].ctypes.data, mpIndex1[0][io:io + fragmentLen].nbytes)

                stepTs2 = subseqLen - 1

                for jo in range(0, muSigma2.shape[1], fragmentLen):

                    ctypes.memmove(cTs2, ts2[0][jo + stepTs2 - (subseqLen - 1):jo + stepTs2 + fragmentLen].ctypes.data, ts2[0][jo + stepTs2 - (subseqLen - 1):jo + stepTs2 + fragmentLen].nbytes)
                    ctypes.memmove(cMu2, muSigma2[0][jo:jo + fragmentLen].ctypes.data, muSigma2[0][jo:jo + fragmentLen].nbytes)
                    ctypes.memmove(cSigma2, muSigma2[1][jo:jo + fragmentLen].ctypes.data, muSigma2[1][jo:jo + fragmentLen].nbytes)
                    ctypes.memmove(cMp2, mp2[0][jo:jo + fragmentLen].ctypes.data, mp2[0][jo:jo + fragmentLen].nbytes)
                    ctypes.memmove(cMpIndex2, mpIndex2[0][jo:jo + fragmentLen].ctypes.data, mpIndex2[0][jo:jo + fragmentLen].nbytes)

                    lib.cudaRun_Union(cSubseqLen, cTs1, cMu1, cSigma1, cMp1, cMpIndex1, cTs2, cMu2, cSigma2, cMp2, cMpIndex2)

                    mp2[0][jo:jo + fragmentLen] = np.ctypeslib.as_array(cMp2)
                    mpIndex2[0][jo:jo + fragmentLen] = np.ctypeslib.as_array(cMpIndex2)

                    stepTs2 = stepTs2 + subseqLen -1

                mp1[0][jo:jo + fragmentLen] = np.ctypeslib.as_array(cMp1)
                mpIndex1[0][jo:jo + fragmentLen] = np.ctypeslib.as_array(cMpIndex1)
